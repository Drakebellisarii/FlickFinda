...
#def add_to_ratings():
   ## try:
        ##print("Received ratings/add request") # Debug print
        ...
        ## print(f"Adding movie: {movie_title} with poster {poster_url}")  # Debug print
        
        ## existing_movie = MovieRating.query.filter_by(movie_title=movie_title).first()
        ## if existing_movie:
            ## print(f"Movie {movie_title} already exists in ratings")  # Debug print
          ###  return jsonify({'message': 'Movie already in watched list', 'success': False}), 400
        
        ##new_movie = MovieRating(
       ...
       ## db.session.commit()
        ...
        ##print(f"Successfully added {movie_title} to ratings")  # Debug print
       ## return jsonify({'message': 'Movie added to watched list', 'success': True}), 200
    
    ##except Exception as e:
       ## print(f"Error in add_to_ratings: {str(e)}")  # Debug print
        ##db.session.rollback()
...
def add_to_ratings():
    try:
        ...
        # Check if movie already exists in user's ratings
        existing_movie = MovieRating.query.filter_by(user_id=user_id, movie_title=movie_title).first()
        if existing_movie:
            return jsonify({'success': False, 'message': 'Movie already in watched list'}), 400
        ...
        return jsonify({'success': True, 'message': 'Movie added to watched list'}), 200
    except Exception as e:
        db.session.rollback()
...
def index():
    ...
    if 'user_id' in session:
        ...
        return render_template('index.html', youtube_api_key=YOUTUBE_API_KEY)
    elif 'is_guest' in session:
        print("Guest session detected")
        # Always pass the YouTube API key to the template
        return render_template('index.html', youtube_api_key=YOUTUBE_API_KEY)
    else:
...
def get_trailer():
    ...
    if movie:
        try:
            ...
            if not items:
                return jsonify({'success': False, 'error': 'No trailer found'})

            video_id = items[0]['id']['videoId']
            print(f"Trailer for '{movie}': videoId={video_id}")
...
def generate_movie_list(description, num_titles=5):
    try:
        ...
        # Retry once if we got fewer results than requested
        ...
        print(f"Error generating movie suggestions: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
